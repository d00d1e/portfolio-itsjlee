...
export default function ProjectCard({ projects }) {
  ...
  const filteredProjects = projects.filter((project) => project.category.includes(filter));

  return (
    <section id="project-card">
      <div id="project-card-filters" className="d-flex flex-row align-items-center w-25 pb-3 pt-4">
        {filters.map((filter) => (
          <button
            ...
            <Zoom key={project._id + filter}>
              <ul className="col-md-6 col-xl-4">
                <li className="card-list">
                  <div className="card">
                    {project.featured && <img className="featured" src={img} alt="featured" />}
                    ...
                    <div className="card-body m-0 pt-2">
                      <div>
                        <p className="card-title text-uppercase h5 m-0">{project.title}</p>
